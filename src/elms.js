...
const CreatedElement = require('./CreatedElement')
const ElementWithAppendedChildren = require('./ElementWithAppendedChildren')
...
tags.forEach(tag => {
  elms[tag] = (attrStr, text) => {
    return (...children) => {
      return new ElementWithAppendedChildren(
        new CreatedElement(tag, attrStr, text), ...children
      )
    }
  }
})
